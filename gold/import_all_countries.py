#!/usr/bin/env python3
"""Import all remaining countries from Excel file into report 1."""
...
from db.database import transaction, execute_query
...
# Get already-imported countries
existing_countries = set()
for row in execute_query("SELECT DISTINCT country FROM apple_keywords"):
    existing_countries.add(row['country'])

print(f"Already imported countries: {', '.join(sorted(existing_countries))}")
...
# Collect keywords for all NEW countries
keywords = []
countries_found = set()
...
print(f"\nFound {len(keywords)} keywords from {len(countries_found)} new countries:")
print(f"  {', '.join(sorted(countries_found))}")
...
print(f"✓ Successfully imported {len(keywords)} keywords from {len(countries_found)} countries to report 1")

# Show final counts
final_counts = {}
for row in execute_query("SELECT country, COUNT(*) as count FROM apple_keywords GROUP BY country ORDER BY count DESC"):
    final_counts[row['country']] = row['count']

print(f"\nTotal countries in database: {len(final_counts)}")
print(f"Total keywords in database: {sum(final_counts.values())}")
