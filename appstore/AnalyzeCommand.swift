...
struct AnalyzeCommand {
    ...
    func execute(options: AnalyzeOptions) async {
        do {
            ...
            let wordVariants = searchWords.map { Self.generateWordVariants($0) }
            ...
            let analyzedApps = apps.prefix(20).map { app in
                Self.analyzeApp(app, searchTerm: options.term, searchWords: searchWords, wordVariants: wordVariants)
            }

            // Step 5: Calculate summary statistics
            let summary = Self.calculateSummary(analyzedApps: analyzedApps)
    ...
    static func generateWordVariants(_ word: String) -> [String] {
        var variants = [word]
    ...
    static func removeFilterWords(_ text: String) -> String {
        let words = text.lowercased().split(separator: " ").map(String.init)
        let filtered = words.filter { !Self.filterWords.contains($0) }
        return filtered.joined(separator: " ")
    }

    static func analyzeApp(_ app: App, searchTerm: String, searchWords: [String], wordVariants: [[String]]) -> AnalyzedApp {
    ...
    static func countWordMatches(in text: String, wordVariants: [[String]]) -> Int {
        var count = 0
    ...
    private func outputCSV(analyzedApps: [AnalyzedApp], summary: SearchSummary, durationMs: Int) {
        ...
        for analyzed in analyzedApps {
            ...
            let ratingCount = app.userRatingCount ?? 0
            let originalRelease = Self.formatDate(analyzed.originalReleaseDate)
            let latestRelease = Self.formatDate(analyzed.latestReleaseDate)
    ...
    static func calculateSummary(analyzedApps: [AnalyzedApp]) -> SearchSummary {
        let totalApps = analyzedApps.count
    ...
    static func calculateCompetitivenessV1(
    ...
    /// Runs the full analysis pipeline and returns structured results.
    /// Used by MCP tool handlers.
    static func runAnalysis(
        term: String,
        storefront: String,
        language: String
    ) async throws -> (analyzedApps: [AnalyzedApp], summary: SearchSummary, apps: [App]) {
        let appIds = try await ScrapeCommand.fetchRankedAppIds(
            term: term,
        ...
        guard !appIds.isEmpty else {
            return ([], SearchSummary(
                avgAgeDays: 0, medianAgeDays: 0, ageRatio: 1.0, avgFreshnessDays: 0,
                avgRating: 0, avgRatingCount: 0, avgTitleMatchScore: 0, avgDescriptionMatchScore: 0,
                avgRatingsPerDay: 0, newestVelocity: 0, establishedVelocity: 0, velocityRatio: 0,
                competitivenessV1: 0, totalTitleScore: 0, totalDescScore: 0, totalRatingsPerDay: 0,
                newestPercentOfRatings: 0, establishedPercentOfRatings: 0, newCount: 0, establishedCount: 0
            ), [])
        }
        ...
        let searchWords = term.lowercased().split(separator: " ").map(String.init)
        let wordVariants = searchWords.map { generateWordVariants($0) }

        let analyzedApps = apps.prefix(20).map { app in
            analyzeApp(app, searchTerm: term, searchWords: searchWords, wordVariants: wordVariants)
        }
        ...
        do {
            ...
            // Database save failure is non-fatal for MCP
        }

        return (analyzedApps, summary, apps)
    }
